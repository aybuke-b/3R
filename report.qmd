...
Dans la littérature, une spécification *semi-log* est généralement préférée afin d'améliorer l'ajustement du modèle $-$ voir @bello2010.
...
+ \beta_{11i}das\_limbs_i + \beta_{12i}das\_chest_i + u_i
\end{split}
$$ {#eq-level-hedonic}
...
+ \beta_{11i}fast\_charging_i + \beta_{12i}network_i + \beta_{13i}ppi_i + u_i
\end{split}
$$ {#eq-log-hedonic}
...
L'hypothèse **(1)** est vérifiée par la formulation de notre modèle. Pour l'hypothèse **(2)**, on peut effectuer un test de *Student* pour vérifier si $E(u_i) = 0$. Les hypothèses $H_0$ et $H_1$ sont les suivantes : 
...
- *Note* : La **Tolérance** correspond à $\frac{1}{\text{VIF}}$, c'est à dire que plus le *VIF* sera élevé, et plus la **Tolérance** sera proche de 0.
...
```{r scf_results}
scf_summary$mleParam |>
    kbl(booktabs = TRUE, digits = 4, caption = "Résultats de l'estimation du modèle SFA (Cost Frontier).") |>
    kable_styling(latex_options = c("striped", "hold_position")) |>
    row_spec(0, bold = T)
```
...
Note $\Rightarrow$ Mettre les 5 téléphones les plus efficients et les 5 téléphones les moins efficients, ainsi que leurs caractéristiques principales.
