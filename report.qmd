...
3. Une combinaison de biens peut procurer une utilité qui ne serait pas la somme des utilités procurées par les biens séparément.
...
quantité maximum d’output $y$ pouvant être produite à partir de vecteurs d'inputs.
...
Avant les travaux de @aigner1977, les économètres utilisaient principalement des fonctions de production pour étudier le lien entre le niveau de production et les quantités d'inputs utilisées dans la littérature. Cela signifie que la formulation théorique énoncée par @farrell1957 différait de son utilisation empirique. En effet, @farrell1957 a introduit la notion d'efficacité au sens du niveau maximum de production qu'on devrait obtenir étant donné une combinaison spécifique d'inputs.
...
La spécification de cette méthode permet d'estimer, à travers l'espérance conditionnelle $u_i$ sachant $\epsilon_i$, les scores de l’**efficacité technique** de chaque firme.
...
$\Rightarrow$ La meilleure réputation de Toyota lui a permis de vendre 200000 voitures à $\$$ 11100, contre seulement 80000 véhicules à $\$$ 10700 vendu pour General Motors.
...
On peut aussi tester l'asymétrie de la distribution avec le coefficient d'asymétrie de *Pearson* : 
...
**Il existe néanmoins plusieurs problèmes dans ces méthodes.**
...
Il existe néanmoins une dernière approche : les *algorithmes génétiques*. Contrairement à la *forward selection* et la *backward elimination* qui sont des méthodes déterministes, les algorithmes génétiques sont eux stochastiques.
...
**$R^2_{adj} = 0.929$ donc 92.3% de la variance de la variable expliquée (log(price)) est expliquée par la variance des variables explicatives du modèle.**

\newpage

### Vérification des hypothèses

Un aspect crucial lors de la construction des modèles de régressions linéaires est d’évaluer la qualité de l’ajustement du modèle, mais aussi de vérifier les hypothèses de **Gauss-Markov** :
...
scf_results |>
    kbl(booktabs = TRUE, caption = "Résultats de l'estimation du modèle SFA (Cost Frontier).", col.names = c("Variable", "Coefficient", "Significativité")) |>
    kable_styling(latex_options = c("striped", "hold_position")) |>
    row_spec(0, bold = T) 
```
...
**APPLE** fait partie du Top 3 des marques les plus efficientes, derrière **NOTHING** et **FAIRPHONE** (Il faut néanmoins se souvenir que ces 2 marques ne proposent que très peu de téléphones, contre 105 pour **APPLE**).
...
new_df |>
    ...
    arrange(efficiency) |>
    slice_head(n = 5) |>
    kbl(booktabs = TRUE, digits = 3, caption = "Les 5 téléphones les moins efficients.") |>
    kable_styling(latex_options = c("striped", "hold_position")) |>
    row_spec(0, bold = T)
```
...
new_df |>
    ...
    arrange(efficiency) |>
    slice_tail(n = 5) |>
    kbl(booktabs = TRUE, digits = 3, caption = "Les 5 téléphones les plus efficients.") |>
    kable_styling(latex_options = c("striped", "hold_position")) |>
    row_spec(0, bold = T)
```
