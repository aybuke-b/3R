...
scree <- factoextra::fviz_screeplot(afdm_res, barfill = highlight, barcolor = highlight, addlabels = TRUE, main = "", ylim = c(0, 25), ylab = "")
...
# mean_eff = mean(efficiency_1),
# median_eff = median(efficiency_1),
# mean(price_diff),


# globalement on voit que les téléphoens dotn le prix a baissé en période 2 ont une efficacité moins importante que les téléphones dont le prix a augmenté
```
...
clean_data |>
    ...
    kbl(caption = "", booktabs = T, col.names = c("Groupe", "Efficacité moyenne", "Efficacité médiane"), escape = F) |>
    kable_styling(latex_options = c("striped", "hold_position")) |>
    row_spec(0, bold = T)
```

Certains téléphones ont augmenté en valeur entre les périodes 1 et 2, ce qui se traduit par une efficacité moyenne de 0.850. D'autres ont vu leurs prix diminuer en période 2, entraînant une efficacité moyenne légèrement inférieure de 0.836. En revanche, pour les téléphones dont le prix est resté constant, l'efficacité moyenne atteint 0.867.
...
::: {.callout-warning}
**Ajouter les 5 téléphones dont la différence de prix est la plus élevée (price_diff), reformater le tableau median/mean efficiency avec kable, et ajouter la petite interpr"tation aussi.**
:::
