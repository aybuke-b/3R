...
Enfin, la dernière étape visuelle que l'on peut mettre en œuvre est de vérifier l'ajustement du modèle. Une vérification prédictive a posteriori est la comparaison entre ce que prédit le modèle ajusté et les données réellement observées.
...
loghedonic_results <- cbind(
    round(loghedonic_summary$coefficients[, 1], 5),
    stars(loghedonic_summary$coefficients[, 4]),
    round(loghedonic_summary$coefficients[, 2], 5)
)
...
```{r worst}
new_df |>
    select(model, efficiency, ram, storage, screen_size, price, logprice, prediction) |>
    arrange(efficiency) |>
...
new_df |>
    select(model, efficiency, ram, storage, screen_size, price, logprice, prediction) |>
    arrange(efficiency) |>
    mutate(price = paste(price, "€"), prediction = paste(prediction, "€")) |>
    slice_tail(n = 5) |>
