...
Il n'existe **évidemment** pas de données directement disponibles recoupant le prix et l'ensemble des caractértiques des smartphones. Ce qui pourrait le plus s'en rapprocher sont les fiches techniques de téléphones disponibles sur <https://www.01net.com/>. Scraper ce site pourrait être une idée intéressante, mais *01net* n'est pas un revendeur de smartphones. 
...
```{r data import}
root <- getwd()
df_path <- file.path(root, "Web_Scraping", "data", "df_clean.csv")
df <- read_delim(df_path, delim = ";")
```

Il y a `r nrow(df)` smartphones valides disponibles dans nos données scrapées sur Boulanger avec 35 variables.
...
On peut s'intéresser au prix moyen par marque pour regarder si il existe des différences de prix significatives entre certaines marques pour illustrer l'article de @boistel2008 cité précédemment.
...
## Etudes des variables catégorielles importantes
...
- On retrouve la même tendance au niveau des parts de marché mondiale des smartphones par rapport à Q3 2022, c'est à dire que Samsung, Apple & Xiaomi se partagent respectivement 22, 18 et 14% de parts de marché.
...
M <- df |>
    select(where(is.numeric), -c(sensor, random_col, logprice)) |>
    cor()
...
cor_height <- round(cor(df$price, df$height), 3)
```

On s'aperçoit que les corrélations les plus importantes avec **price** sont respectivement la capacité de stockage avec un coefficient de corrélation $r_{price, storage}=$ `r cor_storage` et la RAM avec $r_{price, ram}=$ `r cor_ram`. Inversement, la hauteur du téléphone, par exemple, semble ne pas avoir d'incidence sur le prix avec $r_{price, height}=$ `r cor_height`.
...
Nous allons tout d'abord retirer quelques variables qui ne nous seront pas utiles pour l'analyse et qui risquent de complexifier les régressions : *cpu*, *model*, *sensor*, *screen_tech*, *stars*, *reviews*, *color*, *cam_1*, *cam_2*, *cam_3*. Par exemple, il y a presque autant de modèles distincts que de nombre d'observations, et *reviews* et *stars* sont si peu corrélées à la variable à expliquer **price** que les ajouter ne semble pas nécessaire.
...
Cela signifie que l'estimation du paramètre $\gamma$ est soit proche de la limite inférieure de son espace de paramètres (0), soit proche de la limite supérieure de son espace de paramètres (1). Par exemple, un $\gamma$ proche de zéro indique qu'il n'y a preque aucune inefficacité, donc qu'il serait possible d'estimer le modèle par MCO, tandis qu'un $\gamma$ proche de un indique qu'il n'y a presque aucun bruit, donc qu'il serait possible d'utiliser une DEA. Plus globalement, cela indique une mauvaise spécification du modèle ou que d'autres modèles pourraient être plus appropriés dans ce cadre.
:::
...
Un aspect crucial lors de la construction des modèles de régression linéaire est d’évaluer la qualité de l’ajustement du modèle, mais aussi de vérifier les hypothèses de **Gauss-Markov** :

1. Linéarité dans les paramètres.
2. Les résidus sont d'espérance nulle.
3. Absence de multicolinéarité.
4. Homoscédasticité des résidus.
5. Pas d'autocorrélation des résidus.

**Sous toutes ces hypothèses sont satisfaites, l'estimateur fourni par les MCO est dit BLUE (Best Linear Unbiased Estimator).** Pour les vérifier, on peut utiliser les packages `{performance}` et `{see}` en conjonction[^9].
...
- Enfin, des valeurs de *VIF* supérieures à 10 sont le signe d'une corrélation très elevée.
...
Enfin, la dernière étape visuelle que l'on peut mettre en oeuvre est de vérifier l'ajustement du modèle. Une vérification prédictive a posteriori est la comparaison entre ce que prédit le modèle ajusté et les données réellement observées. 

$\Rightarrow$ **L’objectif est de détecter si le modèle est inadéquat pour décrire les données.**

```{r posterior, fig.width=8}
check_posterior_predictions(loghedonic)
```

- On s'aperçoit que le modèle a du mal à représenter le second pic de la distribution des prix. Globalement, les prédictions sont assez proches de la réalité.

## Modèle SFA - Frontière de coût

Le modèle reste le même que celui de l'@eq-log-hedonic, mais le terme d'erreur $\epsilon_i$ devient un terme d'erreur composé $u_i + v_i$. 

Pour modéliser cette frontière de coût, il suffit d'utiliser l'argument `ineffDecrease = FALSE`, autrement dit l'inefficacité augmente la variable endogène.
...
**Les résultats de l'estimation sont disponibles sur la page suivante.**
...
```{r scf_results}
scf_results <- cbind(
    round(scf_summary$mleParam[, 1], 5),
    stars(scf_summary$mleParam[, 4]),
    round(scf_summary$mleParam[, 2], 5)
)

scf_results |>
    kbl(booktabs = TRUE, caption = "Résultats de l'estimation du modèle SFA (Cost Frontier).", col.names = c("Variable", "Coefficient", "Significativité", "SE")) |>
    kable_styling(latex_options = c("striped", "hold_position")) |>
    row_spec(0, bold = T)
```
...
- En ayant comme catégorie de référence *Apple* pour la variable marque (*brand*), on peut voir que toutes les marques ont un impact négatif sur le prix, *cet. par.* 
    - La marque la plus valorisée derrière *Apple* est *Samsung* avec une différence de prix de $22.6\%$.
    - La marque la moins valorisée est *Honor* avec une différence de prix de $102\%$ par rapport à *Apple*.

- Si la *ram* augmente de 1 Go, alors le prix augmente de $8.3\%$, *cet. par.*

- Si le téléphone dispose d'une charge à induction (*induction = `TRUE`*), alors le prix augmente de $28.6\%$, *cet. par.*

- Si la taille de l'écran augmente de 1 pouce (*screen_size*), alors le prix augmente de $24.9\%$, *cet. par.*

- Pour le lieu de fabrication (*made_in*), la catégorie de référence est la *Chine*.
    - Le coefficient associé à la catégorie *Inde* et *Viêt Nam* n'est pas significatif.
    - Comparé à un téléphone produit en *Chine*, un téléphone produit en *Thaïlande* augmente le prix de $55.6\%$, suivi de *Taïwan* avec une augmentation de $51\%$ et enfin du *Japon* avec une augmentation du prix de $41.6\%$, *cet. par.*

- Si le téléphone dispose d'un moyen d'augmenter sa capacité de stockage (*upgrade_storage = `TRUE`*), alors le prix diminue de $33.8\%$, *cet. par*.

- Pour les variables liées au DAS :
    - l'augmentation d'une unité de Watts par kilogramme du *das_limbs* augmente le prix de $8.5\%$, *cet. par.*
    - au contraire, le prix diminue de $20.5\%$ pour le *das_head* et de $27\%$ pour le *das_chest* lorsque qu'ils augmentent d'une unité de W/kg, *cet. par.*

- Si le téléphone dispose d'une charge rapide (*fast_charging = `TRUE`*), alors le prix diminue de $15.9\%$, *cet.par.* 
...
En utilisant l'espérance conditionnelle $E(u_i | \epsilon_i)$, on peut estimer le score d'efficacité pour chaque observation, et donc déterminer quels téléphones sont les plus/moins efficaces de notre sélection. On peut aussi les regrouper par marque.

```{r eff_brands}
eff_df <- as_tibble(efficiencies(scf))
new_df <- bind_cols(df, eff_df)
...
On voit que la marque la moins valorisée par les consommateurs (*Honor*) n'est pas forcément celle qui propose les téléphones les plus inefficaces. 
...
```{r worst}
new_df |>
    select(model, efficiency, ram, storage, screen_size, price) |>
    arrange(efficiency) |>
...
- Avec une efficacité de 0.6 et un prix de 853.6 €, le *MOTOROLA Razr* est très loin de la frontière de coût : son prix est  40% *trop cher*.

- Le *Samsung XCOVER 5* est quant à lui la preuve que même un téléphone moyen de gamme d'une marque comme *Samsung* peut être un mauvais rapport qualité-prix.
...
- l'*ASUS ROG Phone 6* possède un très bon score d'efficacité, proche de 0.95. Avec un prix de 799 €, ce téléphone est donc un excellent rapport qualité-prix. Cela peut s'expliquer par sa capacité de stockage de 512 Go et son importante RAM de 16 Go.

- Le *XIAOMI Redmi Note 10* est quant à lui le téléphone le plus efficace de notre échantillon. De manière intéressante, son prix est très bas.
...
- Parmi ces téléphones les moins chers, on remarque qu'il y a 4 modèles de marque *Xiaomi*. Cela est cohérent avec nos observations dans la partie de statistiques descriptive.

- Par ailleurs, les 5 téléphones les moins chers sont en dessous de la barre symbolique des 100 € ; pour autant, leur efficacité est $> 0.9$ pour l'ensemble des modèles. Les téléphones peu chers ne sont donc pas forcément signe d'un mauvais *rapport "qualité-prix"* !
...
```{r todos}
# Faire une partie avec effets d'interaction plus tard.
# découpage en variables dichotomiques, comparaison résultats régressions hédoniques, etc.
```
...
:   Somme des résolution des caméras 1, 2 et 3 en mégapixels $\Rightarrow$ `ìnt`
