...
Enfin, la dernière étape visuelle que l'on peut mettre en œuvre est de vérifier l'ajustement du modèle. Une vérification prédictive a posteriori permet de simuler des données répliquées avec le modèle ajusté puis les comparer aux données observées.
...
On peut aussi s'intéresser à l'étude des coefficients obtenus par le modèle de régression log-hédonique et les coefficients obtenus par le modèle SFA. On remarque globalement que la significativité est la même et que la valeur des coefficients est similaire
...
scf_results_2 <- scf_results[1:30, 1:3]
...
variance_plot <- function(pca_results, color) {
    eigenvalues <- pca_results$eig |>
        as_tibble() |>
        rename(var = `percentage of variance`, cum_var = `cumulative percentage of variance`)

    eigenvalues <- eigenvalues |> mutate(highlight = if_else(row_number() %in% c(1, 2), TRUE, FALSE))
...
cor_circle <- function(pca_results) {
    var_names <- rownames(pca_results$var$coord)
    pca_vars <- pca_results$var$coord |> as_tibble()
    pca_vars <- pca_vars |> add_column(variable = var_names, .before = "Dim.1")
...
```{r}
pca_df <- new_df |> select(brand, ram, storage, logprice, efficiency, screen_size)

pca_res <- PCA(pca_df, quali.sup = 1, scale.unit = TRUE)

# storage + brand + ram + induction + screen_size + made_in + upgrade_storage + das_head + das_limbs + fast_charging + das_chest + network + ppi

afdm_df <- new_df |> select(efficiency, logprice, fast_charging, upgrade_storage, induction, network, ram, storage, brand)

# mca_df <- mca_df |> mutate(ram = as_factor(ram), storage = as_factor(storage))

afdm_res <- FAMD(mca_df, ncp = 5)
...
#| fig-align: center
#| fig-cap: "Inerties."
variance_plot(afdm_res, highlight)
...
- le nuage des individus avec les variables qualis sup + les contributions.

```{r}
#| fig.width: 4.5
#| fig.height: 4.5
#| fig-align: center
#| fig-cap: "Cercle des corrélations."
cor_circle(pca_res)
```
