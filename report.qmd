...
thematic::thematic_on(fg = "black", bg = "white", accent = "purple", font = "PT Sans")
...
On voit que dans le modèle de l'@eq-level-hedonic, il n'y a aucun effet d'interaction ou d'effet non-linéaire. Pour autant, les résultats sont déjà **très satisfaisants** avec un $R^2_{adj} > 0.8$.
...
Ainsi, on remarque que 6 variables sont présentes dans l'ensemble des modèles, donc ces variables sont très importantes pour notre modélisation. Ensuite, trois variables sont au-dessus de la ligne de *cutoff* mais inférieure à 1 : *usb_type_c*, *upgrade_storage* et *made_in*. L'ensemble des autres variables sont comprises entre 0 et 0.25. On peut donc considérer que l'importance de ces variables dans notre modèle est négligeable.
...
<!---   Lorsque le *ppi* (*pixels par pouce*) augmente d'une unité, alors le prix diminue de 5.52 €, *cet. par.* -->
...
<!---   Si la taille de l'écran augmente de 1 pouce, alors le prix diminue de 345.2 €, *cet. par.* -->
...
-   Si le téléphone dispose d'une USB type C (*usb_type_c = `TRUE`*), alors le prix augmente de 89.83 €, *cet.par.*
...
-   Si le téléphone dispose d'un moyen d'augmenter sa capacité de stockage (*upgrade_storage* = `TRUE`), alors le prix diminue de 123.04 €, *cet.par* ; comme nous l'avions remarqué dans la partie de statistiques descriptives.

<!--- Pour les variables concernant la résolution de l'écran : 
    -  Si la résolution de l'écran (*resolution_1*) augmente de 1 pixel dans la première dimension, alors le prix du téléphone augmente de 0.698 €, *cet. par.*
    - Si la résolution de l'écran (*resolution_2*) augmente de 1 pixel dans la deuxième dimension, alors le prix du téléphone augmente de 0.853 €, *cet. par.*-->



$R^2_{adj} = 0.877$ donc 87.7% de la variance de la variable expliquée (price) est expliquée par la variance des variables explicatives du modèle.

\newpage

### Vérification des hypothèses

Un aspect crucial lors de la construction des modèles de régression linéaire est d’évaluer la qualité de l’ajustement du modèle, mais aussi de vérifier certaines hypothèses comme l'espérance nulle des résidus, leur homoscédasticité, l'absence de multicolinéarité, etc.
...
Commençons par nous intéresser à la multicolinéarité dans notre régression.
...
```{r collinearity_level, fig.width=6}
check_collinearity(genreg) |>
...
#| fig.width: 8
#| fig-align: center
performance::check_posterior_predictions(genreg)
```
...
```{r hyp2_level}
pvalue_hyp2_level <- t.test(genreg$residuals)$p.value
```
...
ggplot(genreg, aes(x = .fitted, y = .resid)) +
    geom_point() +
...
```{r bp_test_level}
bp_pvalue_level <- round(bptest(genreg)$p.value, 2)
```

\vspace{1em}

-   La $p-value$ issue du test est égale à `r bp_pvalue_level`, donc l'hypothèse d'homoscédasticité des résidus n'est pas vérifiée.

**Intéressons-nous également à l'influence des valeurs extrêmes dans notre régression.**

```{r outliers_level, fig.width=6, fig.height=3}
#| fig-align: center
#| fig-cap: "Influence des valeurs extrêmes."
outliers <- check_outliers(genreg)
...
```{r normality_level, fig.width=6, fig.height=2.75}
#| fig-align: center
#| fig-cap: "Histogramme des résidus avec loi normale superposée."
mean_resid <- mean(genreg$resid)
sd_resid <- sd(genreg$resid)

ggplot(genreg, aes(x = .resid)) +
    geom_histogram(aes(y = after_stat(density)), bins = 40, fill = "black", color = "white", alpha = 0.7) +
    stat_function(fun = dnorm, args = list(mean = mean_resid, sd = sd_resid), color = "darkgreen", size = 1) +
    labs(x = "", y = "Densité")

skewness_genreg <- skewness(genreg$residuals)
```

On voit que le coefficient d'asymétrie est positif **(`r round(skewness_genreg,2)`)**. Cette asymétrie indique une concentration de valeurs sur le côté gauche de la distribution, on peut d'ailleurs l'observer sur l'histogramme.
...
-   La $p-value$ issue du test est égale à `r pvalue_hyp2_level`, donc on rejette $H_0$. Les résidus ne sont donc pas ici d'éspérance nulle, ce qu'on avait pu confirmer graphiquement plus haut.
...
-   Si la taille de l'écran augmente de 1 pouce (*screen_size*), alors le prix augmente de $13.4\%$, *cet. par.*

-   Pour le type d’écran (*screen_type*), la catégorie de référence est *borderless* (un écran sans bordure) :
    - Les coefficients associés à la catégorie *screen_typeBords incurvés* et *screen_typePlat* ne sont pas significatifs.
    <!--- Disposer d’un écran à bord incurvé (*screen_typeBords incurvés*) diminue le prix de $10.9\%$ par rapport à la catégorie de référence, *cet. par*.
    - Pour l’écran plat (*screen_typePlat*), le prix diminue de $3.5\%$ par rapport à un écran sans bordure, *cet. par*.-->
...
-   Pour le lieu de fabrication (*made_in*), la catégorie de référence est la *Chine*.
   ...
   <!-- -   Un téléphone qui est fabriqué au *Viêt Nam* est $9.3\%$ moins cher qu'un téléphone fabriqué en *Chine*, *cet. par.*-->
    -   Comparé à un téléphone produit en *Chine*, un téléphone produit en *Thaïlande* augmente le prix de $69.8\%$, *cet. par.*

-   Si le téléphone dispose d'un moyen d'augmenter sa capacité de stockage (*upgrade_storage = `TRUE`*), alors le prix diminue de $28.2\%$, *cet. par*.

-   Pour la variable liée au DAS :

    -   l'augmentation d'une unité de Watts par kilogramme du *das_limbs* augmente le prix de $9.7\%$, *cet. par.*
...
$R^2_{adj} = 0.923$ donc 92.3% de la variance de la variable expliquée est expliquée par la variance des variables explicatives du modèle.

\newpage

### Vérification des hypothèses

```{r collinearity_log, fig.width=6}
...
```{r bp_test_log}
bp_pvalue_level <- round(bptest(loghedonic)$p.value, 2)
```

-   En effet, la $p-value$ issue du test est égale à `r bp_pvalue_level`, donc l'hypothèse d'homoscédasticité des résidus est vérifiée.

**On peut alors se pencher sur l'influence des valeurs extrêmes dans notre régression.**

```{r outliers_log, fig.width=6, fig.height=3}
#| fig-align: center
#| fig-cap: "Influence des valeurs extrêmes."
...
```{r normality_log, fig.width=6, fig.height=2.5}
#| fig-align: center
#| fig-cap: "Histogramme des résidus avec loi normale superposée."
...
On voit que le coefficient d'asymétrie est toujours positif **(`r round(skewness_loghedonic,2)`)**. 

@kumbhakar2015 mentionnent l'utilisation de ce coefficient d'asymétrie comme *"test"* préalable à la spécification d'une **SFA**.
...
- **gammaVar** correspond à la part de variance totale qui est due à l'inefficacité. Dans notre cas, 37,5% de la variance totale est due à l'innefficacité.
...
- Si la taille de l'écran augmente de 1 pouce (*screen_size*), alors le prix augmente de $13.26\%$, *cet. par.*

-   Pour le type d’écran (*screen_type*), la catégorie de référence est *borderless* (un écran sans bordure) :
    - Les coefficients associés à la catégorie *screen_typeBords incurvés* et *screen_typePlat* ne sont pas significatifs.
    <!--- Disposer d’un écran à bord incurvé (*screen_typeBords incurvés*) diminue le prix de $8.97\%$ par rapport à la catégorie de référence, *cet. par*.
    - Pour l’écran plat (*screen_typePlat*), le prix diminue de $2.62\%$ par rapport à un écran sans bordure, *cet. par*.-->
...
-   Si le téléphone dispose d'un moyen d'augmenter sa capacité de stockage (*upgrade_storage = `TRUE`*), alors le prix diminue de $26.19\%$, *cet. par*.

-   Pour la variable liée au DAS :

    -   L'augmentation d'une unité de Watts par kilogramme du *das_limbs* diminue le prix de $12.15\%$, *cet. par.*
...
#| label: fig-efficiency-smartspecs
...
scree <- factoextra::fviz_screeplot(afdm_res, barfill = highlight, barcolor = highlight, addlabels = TRUE, main = "", ylim = c(0, 35), ylab = "")
...
- Les variables contribuant le plus à la dimension 1 sont *logprice*,  *upgrade_storage*, *induction*, *ppi*, *network*, et *brand*.
...
D'après le nuage des variables, on peut voir que les variables qui contribuent à l'axe $F_1$ sont le *logprice*, *ppi*, *storage* ainsi que *ram*. On peut donc supposer que les 3 caractéristiques constituent la majeure partie de la variation observée dans la variable *logprice*.

Pour l’axe $F_2$, ce sont principalement les variables *screen_size* et *efficiency* qui contribuent le plus à cet axe. On peut observer que l'efficacité (*efficiency*) est moins bien représentée que *screen_size*, comme le suggère la différence dans la longueur des flèches associées à ces variables.
...
L'ensemble des variables contribue assez peu aux dimensions 3 et 4.
Pour l'axe $F_3$, la *ppi* contribue de manière plus importante, suivie ensuite de *storage* et *ram*.

Pour l'axe $F_4$, c'est la variable *screen_size* qui contribue le plus, suivie par *efficiency*, tandis que la contribution de *logprice* est négligeable. Il est également à noter que toutes ces variables sont positionnées du côté négatif de l'axe.

# Analyse des évolutions tarifaires sur deux périodes

En référence à l'étude d'@arrondo2018, et disposant de données respectivement d'octobre 2023 et de février 2024, nous avons souhaité examiner si les prix des téléphones disponibles sur ces deux périodes ont diminué ou non. Cette analyse vise à identifier les téléphones affichant des tarifs excessifs.

Il est à noter qu'une telle analyse est assez complexe techniquement. En effet, aucune clé unique identifiable n'existe pour distinguer les téléphones, il faut donc en créer une **artificiellement**, comme suit :
...
df_path_1 <- file.path(root, "Web_Scraping", "data", "df_clean.csv")
df_path_2 <- file.path(root, "Web_Scraping", "data", "df_clean_2.csv")

df_1 <- read_delim(df_path_1, delim = ";")
df_2 <- read_delim(df_path_2, delim = ";")

scf_period_1 <- sfa(
    log(price) ~ storage + brand + ram + induction + screen_size
        + made_in + upgrade_storage + das_limbs + network + ppi,
    data = df_1,
    truncNorm = FALSE,
    ineffDecrease = FALSE
...
period_1 <- bind_cols(efficiencies(scf_period_1) |> as_tibble(), period_1)
...
periods <- bind_rows(period_1, period_2)

periods <- periods |> unite("unique_key", c(model, color, ram, storage), sep = "_", remove = FALSE)

# on fait un group by par clé unique,
# puis un distinct de période.
# Quand ce distinct = 2, alors le téléphone existe dans les deux périodes !

valid <- periods |>
    group_by(unique_key) |>
    summarise(periods_exist = n_distinct(period)) |>
    filter(periods_exist == 2) |>
    select(unique_key) |>
    pull()

valid_v2 <- periods |>
    filter(unique_key %in% c(valid)) |>
    group_by(unique_key) |>
    summarise(nb = n()) |>
    filter(nb == 2) |>
    select(unique_key) |>
...
dt <- periods |>
    filter(unique_key %in% c(valid_v2)) |>
    arrange(unique_key)

clean_data <- pivot_wider(
    dt,
    names_from = period,
    values_from = c(price, logprice, efficiency),
    names_sep = "_"
) |>
    mutate(
        price_diff = price_1 - price_2,
        group = case_when(
            price_diff < 0 ~ "+ Period 2",
            price_diff == 0 ~ "= Period 2",
            price_diff > 0 ~ "- Period 2"
        )
    ) |>
...
n_prices <- clean_data |>
    select(model, efficiency_1, price_1, price_2, price_diff, group) |>
    arrange(price_diff) |>
    group_by(group) |>
    summarise(n = n())

prices_less <- n_prices |>
    filter(group == "- Period 2") |>
    pull()
prices_equal <- n_prices |>
    filter(group == "= Period 2") |>
    pull()
prices_more <- n_prices |>
    filter(group == "+ Period 2") |>
    pull()
```

En concaténant ces 4 variables, la clé unique pour chaque téléphone entre les 2 périodes est créée, et après un certain nombre d'opérations de manipulation de données, il en résulte que **`r clean_data |> nrow()`** téléphones sont présents en période 1 et 2. Parmi ces téléphones, **`r prices_less`**, soit `r (prices_less/clean_data |> nrow())*100` %, ont vu leur prix diminuer en période 2, **`r prices_equal`** ont le même prix qu'en première période, et enfin, **`r prices_more`** téléphones ont un prix plus important en période 2. 

Contre-intuitif à première vue, si le prix de certains téléphones est plus élevé en seconde période, c'est probablement car ils disposaient d'une promotion en première période.

On peut également regarder l'efficacité moyenne des groupes en fonction de si leur prix a baissé, augmenté ou est resté stable entre les deux périodes.
...
clean_data |>
    ...
    group_by(group) |>
    summarise(
        mean_eff = round(mean(efficiency_1), 3),
        median_eff = round(median(efficiency_1), 3)
    ) |>
    kbl(caption = "Efficacité par groupe de variation de prix", booktabs = T, col.names = c("Groupe", "Efficacité moyenne", "Efficacité médiane"), escape = F) |>
    kable_styling(latex_options = c("striped", "hold_position")) |>
    row_spec(0, bold = T)
```

Entre les périodes 1 et 2, le prix de certains téléphones a augmenté (groupe **+**). Ces téléphones avaient une efficacité moyenne et médiane de 0.85 et respectivement 0.874 en période 1. D'autres téléphones ont quant à eux vu leur prix diminuer (groupe **-**) en période 2, et leur efficacité moyenne et médiane en période 1 sont inférieures à celles des téléphones dont le prix a augmenté. Cette observation nous indique qu'il semble exister un lien entre l'efficacité (calculée par la SFA en période 1) et la réduction de prix appliquée en période 2.

D'autre part, on peut aussi s'intéresser aux téléphones ayant enregistré la baisse de prix la plus significative.
```{r}
most_exp <- clean_data |>
    select(model, price_1, price_2, price_diff) |>
    arrange(desc(price_diff)) |>
    mutate(price_1 = paste(price_1, "€"), price_2 = paste(price_2, "€"), price_diff = paste(price_diff, "€"))  |>
    slice_head(n = 5) |>
    kbl(caption = "Les 5 téléphones avec les plus grandes variations de prix entre les deux périodes", booktabs = T, col.names = c("Modèle", "Prix période 1", "Prix période 2", "Différence de prix"), escape = F) |>
    kable_styling(latex_options = c("striped", "hold_position")) |>
    row_spec(0, bold = T)
...
- Les téléphones ayant enregistré les plus importantes baisses de prix sont tous des modèles haut de gamme.

- Le Samsung Galaxy Z Fold 4 est le téléphone qui présente la plus forte baisse de prix, passant de 1799 € à 1070.32 €, soit une réduction de presque 730 €.

On peut aussi examiner les modèles de téléphones pour lesquels le prix est resté inchangé entre les deux périodes.

```{r}
prix_stagne <- clean_data |>
    select(model, price_1, price_2, price_diff) |>
    mutate(price_1 = paste(price_1, "€"), price_2 = paste(price_2, "€"))  |>
    arrange(desc(price_diff)) |>
    filter(price_diff == 0) |>
    slice_head(n = 10) |>
    distinct(model, .keep_all = TRUE) |>
    kbl(caption = "5 téléphones avec des prix stables entre les deux périodes", booktabs = T, col.names = c("Modèle", "Prix période 1", "Prix période 2", "Différence de prix"), escape = F) |>
    kable_styling(latex_options = c("striped", "hold_position")) |>
    row_spec(0, bold = T)
...
On observe que les téléphones dont le prix n'a pas changé sont principalement des Motorolas ou, de manière plus générale, des téléphones considérés comme bas de gamme.
...
***Smart Specs*** est plus qu'un simple comparateur de smartphones puisqu'il inclut une dimension de modélisation, et plus précisément un score déterminé par un modèle SFA. De plus, il propose une fonctionnalité unique de comparaison des caractéristiques qui se distingue des autres comparateurs en analysant de manière détaillée les caractéristiques similaires entre deux modèles de téléphones, offrant ainsi une approche point par point.

D'autre part, ***Smart Specs*** offre une panoplie de fonctionnalités variées, telles que : 
...
- un moteur basé sur un modèle **SFA**


L'aspect central de cette application réside dans son onglet "Efficacité". Cet onglet présente deux indicateurs principaux : le nombre de téléphones efficaces et inefficaces. Ces données sont générées à partir d'un nuage de points représentant l'efficacité en fonction du prix, visible ci-dessous : 
...
Les téléphones situés dans la zone rouge sont considérés comme inefficaces en termes de rapport qualité-prix, tandis que ceux situés au-dessus sont considérés comme offrant un bon rapport qualité-prix.

Le seuil d'efficacité, établi à 0.8, est déterminé en se basant sur l'efficacité moyenne ainsi que sur la @fig-efficiency-smartspecs.
Il convient de noter que l'utilisateur a la possibilité d'ajuster ce seuil selon ses préférences pour un contrôle plus personnalisé.

::: {.callout-tip}

## Lien de l'application déployée

L'application est à découvrir en suivant ce lien : [https://smart-specs.streamlit.app/](https://smart-specs.streamlit.app/)
:::
...
Les statistiques descriptives réalisées sur 432 smartphones vendus par un revendeur français *(février 2023)* ont révélé une distribution bimodale des prix, confirmant la nature hautement différenciée des smartphones. Grâce à des modèles de régression hédonique, nous avons réussi à prédire avec précision les prix en utilisant différents types de spécifications, d'abord en *niveau-niveau* puis en *log-niveau*. Ces résultats ont été extrêmement satisfaisants, démontrant la capacité des modèles à correctement inférer les prix des smartphones en fonction de leurs caractéristiques.
...
La dernière étape de ce projet consista à créer une application comparative. Celle-ci permet aux utilisateurs de spécifier les caractéristiques recherchées dans un téléphone. En utilisant le modèle SFA que nous avons développé, un score est généré pour chaque téléphone, classant ainsi les appareils du plus efficace au moins efficace en fonction des critères spécifiés. Cette application offre une solution pratique et personnalisée aux utilisateurs pour prendre des décisions informées lors de l'achat d'un nouveau smartphone.
