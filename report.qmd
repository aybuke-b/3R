...
qt_price <- quantile(df$price, probs = seq(0, 1, 0.1))
```
...
```{r plots, fig.width=8, fig.height=3}
#| fig-align: center
...
## Prix moyen en fonction des caractéristiques
...
df |>
    ...
    kbl(caption = "Prix moyen en fonction de la RAM.", booktabs = T) |>
...
df |>
    ...
    kbl(caption = "Prix moyen en fonction du stockage.", booktabs = T) |>
    kable_styling(latex_options = c("striped", "hold_position")) |>
    row_spec(0, bold = T)
...
- Il existe une relation non-linéaire concernant le doublement du stockage. Par exemple, passer de 64 à 128 Go implique une augmentation du prix moyen de 250% alors que passer de 256 à 512 Go de stockage implique seulement 141% d'augmentation du prix moyen

```{r fig.width=8}
#| fig-align: center
#| fig-cap: "Ridge plot : RAM et Stockage"
...
Faire un giga tableau avec min/max/mean/q1/q2/q3, etc.

Faire des barcharts pour les variables TRUE/FALSE (induction, usb_type_c, fast_charging)
...
Verifier les hypothèses BLUE, hétéroscédasticité, découpage en variables dichotomiques, etc.
