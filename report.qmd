...
- On s'aperçoit que le modèle a du mal à représenter le second pic de la distribution des prix. Globalement, le modèle est assez proche de la réalité.
...
scf_summary
se <- as.vector(coef(summary(scf))[, 2])
coefs <- as.vector(coef(summary(scf))[, 1])
```
