...
Aussi, le prix moyen d'un smartphone dans l'étude est de 136,35 $. Note intéressante, l'**écart-type** du prix des smartphones est particulièrement elevé (181), c'est à dire que la dispersion en prix est très importante, ce qui confirme que les smartphones sont des biens hautement différenciés.
...
- Consiste à générer au hasard une population de solutions candidates pour ensuite leur permettre d’évoluer. Cette évolution se déroule de génération en génération.
...
**TODO : Liste parcimonieuse de variables qui sont déterminantes dans le niveau du prix.**
...
:::{.notes}
Avec 35 prédicteurs, le choix des variables *essentielles* à retenir est important.
...
Deux approches heurisiques similaires (heuristique = élimine progressivement les alternatives) : Backward Elimination, Forward Selection.

- On va décrire plus en détail la Forward Selection.

**MAIS IL Y A PLUSIEURS PROBLEMES DANS CES METHODES** : 

- La Forward & La Backward ne convergent pas tout le temps vers le même modèle.
- Le modèle final n'est pas forcément optimal.

Une autre approche consiste à faire une recherche exhaustive, c'est à dire d'explorer l'*ensemble* des modèles possibles.
C'est une méthode tout à fait faisable quand il y a peu de prédicteurs (moins de 5-6) mais dès qu'il y en a plus on commence à avoir des millions de combinaisons de modèles possibles donc sauf si on veut rester 300 ans devant l'écran il faut oublier.

Autre approche $\Rightarrow$ Les algorithmes génétiques. Contrairement à la stepwise selection qui est déterministe, les algorithmes génétiques sont eux stochastiques (ie. il y a un composant aléatoire)
...
Variables importantes dans la détermination du prix :
